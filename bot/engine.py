...
class TradingApp:
    ...
    def __init__(self, cfg: dict):
        ...
        # === Telegram: adjunta self.telegram y arranca el bot en background ===
        try:
            if run_telegram_bot:
                # engine_api puede ser el propio app: expone get_positions(), toggle_killswitch(), etc.
                run_telegram_bot(self, self.cfg, engine_api=self)
    ...
    def _funding_gate_ok(self, symbol: str, side: str):
        """
        Gate direccional por funding (anualizado, en bps):
          - Bloquea LONG si FR > +limit
          - Bloquea SHORT si FR < -limit
        """
        try:
            fg = self.funding_guard or {}
            if not fg.get("enabled", True):
                return True, ''
            limit = float(fg.get("annualized_bps_limit", 5000))
            fr_bps = float(self.price_cache.get(f"FUNDING_BPS:{symbol}", 0.0))
            if side == 'long' and fr_bps > limit:
                return False, f"REJECT_FUNDING_GATE LONG fr={fr_bps:.0f}bps > {limit:.0f}bps"
            if side == 'short' and fr_bps < -limit:
                return False, f"REJECT_FUNDING_GATE SHORT fr={fr_bps:.0f}bps < -{limit:.0f}bps"
            return True, ''
        except Exception:
            return True, ''
    ...
    async def step_all_symbols(self):
        ...
        for sym in self.symbols:
            ...
            ind = compute_indicators(
                df,
                {**self.filters, **self.strategy_conf, **self.cfg.get("indicators", {})},
            ...
            # funding guard (cache ya calculado). Gate direccional extra
            if bool(self.funding_guard.get("enabled", False)) and sig.side in ("long", "short"):
                ok_fg, reason_fg = self._funding_gate_ok(sym, sig.side)
                if not ok_fg:
                    self.log_decision(sym, 'funding_gate', detail=reason_fg)
                    continue
            ...
            if sig.side in ("long", "short"):
                ...
                # === CAMBIO: Unificar TP en la apertura (sin parciales) ===
                tp_final = sig.tp2 or sig.tp1
                self.trader.open_lot(sym, sig.side, qty, fill.price, lev, sig.sl, tp_final, tp_final, fee,
                                     entry_adx=float(ind.iloc[-1].get('adx', 0.0)), leg=leg_no)

                # === Ajuste TP por target de equity (igual simulador) ===
                try:
                    strat = self.cfg.get("strategy", {})
                    target = strat.get("target_eq_pnl_pct", None)
                    size_mode = (self.cfg.get("risk", {}) or {}).get("size_mode", "full_equity")
                    if target is not None and size_mode == "full_equity":
                        target = float(target)  # ej. 0.10 = 10% del equity
                        eq_now = float(self.trader.equity())
                        # PnL objetivo en USD:
                        pnl_usd = target * eq_now
                        # move = pnl / qty  => TP = entry ± move
                        move = pnl_usd / max(qty, 1e-12)
                        if sig.side.lower() == "long":
                            tp_new = fill.price + move
                        else:
                            tp_new = fill.price - move
                        # Es TP único -> guardarlo en tp2
                        self.trader.state.positions[sym][-1]["tp2"] = float(tp_new)
                except Exception:
                    pass
                ...
                if self.mode == 'real':
                    try:
                        # === CAMBIO: Usar tp_final para protecciones en real ===
                        await self.exchange.place_protections(sym, sig.side, qty, sig.sl, tp_final, tp_final)
                    except Exception as e:
                    ...
                    if sum(len(v) for v in self.trader.state.positions.values()) < self.limits.max_total_positions and len(lots) < self.limits.max_per_symbol:
                        ...
                        if ok_add:
                            ...
                            if qty_add > 0:
                                ...
                                # === CAMBIO: Unificar TP también en DCA ===
                                tp_final_dca = sig.tp2 or sig.tp1
                                self.trader.open_lot(sym, side_net, qty_add, fill2.price, lev, sig.sl, tp_final_dca, tp_final_dca, fee2,
                                                     entry_adx=float(ind.iloc[-1].get('adx', 0.0)), leg=leg_no)

                                # === Ajuste TP por target de equity (igual simulador) - para DCA ===
                                try:
                                    strat = self.cfg.get("strategy", {})
                                    target = strat.get("target_eq_pnl_pct", None)
                                    size_mode = (self.cfg.get("risk", {}) or {}).get("size_mode", "full_equity")
                                    if target is not None and size_mode == "full_equity":
                                        ...
                                        eq_now = float(self.trader.equity())
                                        pnl_usd = target * eq_now
                                        move = pnl_usd / max(qty_add, 1e-12)
                                        if side_net.lower() == "long":
                                            tp_new = fill2.price + move
                                        else:
                                            tp_new = fill2.price - move
                                        self.trader.state.positions[sym][-1]["tp2"] = float(tp_new)
                                except Exception:
    ...
    async def manage_positions(self, price_by_symbol):
        for sym, lots in list(self.trader.state.positions.items()):
            ...
            while idx < len(lots):
                ...
                # =============================================================
                # === CAMBIO: Lógica de gestión de posiciones simplificada ===
                if L['side'] == 'long':
                    if price <= L['sl']:
                    ...
                    # TP ÚNICO
                    if price >= L['tp2']:
                        pnl = self.trader.close_lot(sym, idx, price, fee=fee_unit, note="TP")
                        self.log_trade(sym, L['side'], L['qty'], price, L['lev'], fee_unit, pnl, note="CLOSE_TP")
                        self.save_state()
                        self._tg_close("TP", sym, L, price, pnl)
                        if self.send_legacy:
                            try:
                                await self.notifier.send(f"✅ TP {sym} long qty={L['qty']:.6f} @ {price:.2f} pnl={pnl:.2f} saldo={self.trader.equity():.2f}")
                            except Exception:
                                pass
                        continue
                else:
                    ...
                    if price <= L['tp2']:
                        ...
                        if self.send_legacy:
                            try:
                                await self.notifier.send(f"✅ TP {sym} short qty={L['qty']:.6f} @ {price:.2f} pnl={pnl:.2f} saldo={self.trader.equity():.2f}")
                            except Exception:
                                pass
                        continue
                # =============================================================
    ...
    def pre_open_checks(self, symbol, side, price_by_symbol):
        ...
        if not ok:
            return False, reason
        # funding gate direccional adicional (por si cambia la side entre checks)
        ok, reason = self._funding_gate_ok(symbol, side)
        if not ok:
            return False, reason
        self.save_state()
        return True, ""
